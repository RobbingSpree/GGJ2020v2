...
switch argument0
{
	case item.Grass:			return item.Crop;
	case item.Stick:			return item.Poo_stick;
	case item.Small_animal:		return -1;
	case item.Shrub:			return -1;
	case item.Dirt:				return item.Fetaliser;
	case item.Poo:				return item.Bigger_poo;
	case item.Rock:				return -1;
	case item.Ore:				return -1;
	case item.Sprout:			return -1;
	case item.Coal:				return -1;
	case item.Insect:			return item.Beatle;
	case item.Bee:				return -1;
	case item.Water:			return item.Fetaliser;
	case item.Long_Grass:		return item.Bigger_Crops;
